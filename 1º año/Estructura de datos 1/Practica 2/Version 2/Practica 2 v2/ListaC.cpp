...
bool ListaC::AtenderMayor(Paciente &Per)
{
    TNodo_ListaC *nd=new TNodo_ListaC;
    Paciente p;
    p=primero->Datos;
    ...
    {
        while(nd!=ultimo)///primer paciente es p
        {
            nd=nd->Sig;
            if(p.edad<nd->Datos.edad)
            {
                p.edad=nd->Datos.edad;
                strcpy(p.nombre,nd->Datos.nombre);
                strcpy(p.apellidos,nd->Datos.apellidos);
                p.id=nd->Datos.id;
